...
def get_convex_hull(config:RobotObstacleConfiguration) -> Polygon:
    ...
    new_robot_polygon = affinity.translate(config.robot.polygon, xoff=-last_robot_pose[0], yoff=-last_robot_pose[1])
    # print(list(new_robot_polygon.exterior.coords))
    new_robot_polygon = affinity.rotate(new_robot_polygon, angle=math.radians(-last_robot_pose[2]))

    new_obstacle_polygon = affinity.translate(config.obstacle.polygon, xoff=-last_obstacle_pose[0], yoff=-last_obstacle_pose[1])
    new_obstacle_polygon = affinity.rotate(new_obstacle_polygon, angle=math.radians(-last_obstacle_pose[2]))

    new_obstacle_polygon = affinity.translate(new_obstacle_polygon, xoff=math.sqrt(vector[0]**2 + vector[1]**2), yoff=0)
    total_polygon = new_robot_polygon.union(new_obstacle_polygon)
...
def get_obstacle_pos_from_prev(new_robot_pose:PoseModel, last_robot_pose:PoseModel, last_obstacle_pose:PoseModel) -> PoseModel:
    ...
    theta_rad = math.radians(new_robot_pose[2])

    return (new_robot_pose[0] + length * math.cos(theta_rad), new_robot_pose[1] + length * math.sin(theta_rad),utils.normalize_angle_degrees(new_robot_pose[2] + vector[2]))

def get_motion_vector_to_next(current:PoseModel, next:PoseModel):
    vector = (next[0] - current[0], next[1] - current[1],next[2] - current[2])
    return vector
...
def change_robot_config_with_new_posemodel(pose:PoseModel, config:RobotObstacleConfiguration, converter) -> RobotObstacleConfiguration:
    ...
    vector = get_motion_vector_to_next(config.robot.floating_point_pose, pose)
    ...
    return RobotObstacleConfiguration(
        ...
        robot_fixed_precision_pose=converter(pose),
        ...
        obstacle_fixed_precision_pose=converter(new_obstacle_pose),
        obstacle_polygon=config.obstacle.polygon,
        manip_pose_id=config.manip_pose_id,
        action=ba.Translation((vector[0], vector[1])),
...
class Node:
    ...
    cost: float = 0.0
    def __hash__(self):
        return (self.pose, self.parent.__hash__() if self.parent != None else 0, self.cost).__hash__()
...
class DiffDriveRRTStar:
    def __init__(
        ...
        self.x_center = np.array([(self.start.pose.obstacle.floating_point_pose[0] + self.goal.pose.obstacle.floating_point_pose[0]) / 2,
                                  (self.start.pose.obstacle.floating_point_pose[1] + self.goal.pose.obstacle.floating_point_pose[1]) / 2])
        dx = (self.goal.pose.obstacle.floating_point_pose[0] - self.start.pose.obstacle.floating_point_pose[0]) / self.c_min if self.c_min > 0 else 1
        dy = (self.goal.pose.obstacle.floating_point_pose[1] - self.start.pose.obstacle.floating_point_pose[1]) / self.c_min if self.c_min > 0 else 0
    ...
    def steer(self, from_node: Node, target: RobotObstacleConfiguration) -> Node:
        ...
        linear_vels = np.linspace(-self.max_vel*0.5, self.max_vel, 6)
    ...
    def get_near_nodes(self, node: Node) -> List[Node]:
        ...
        poses = np.array([n.pose.obstacle.floating_point_pose for n in self.tree])
        node_pose = np.array(node.pose.obstacle.floating_point_pose)

        # Compute only (x, y) distance, ignore theta for neighborhood
        diff = poses[:, :3] - node_pose[:3]
    ...
    def plan(self):
        ...
        print(self.start.pose.obstacle.floating_point_pose, self.goal.pose.obstacle.floating_point_pose)
        for n in range(self.max_iter):
            ...
            if self.near_goal(new_node):
                ...
                final_configs[0].action = ba.Grab(self.start.pose.obstacle.action.entity_uid,self.start.pose.obstacle.action.distance)
                final_configs[-1].action = ba.Release(self.start.pose.obstacle.action.entity_uid,self.goal.pose.obstacle.action.distance)
                print("FINAL PATH : ")
                for candidate in final_configs:
                    pass
                    #print(candidate.obstacle.floating_point_pose)
                print("----------------------")
                return (True, final_configs, self.tree, set(list(map(lambda x : x.pose, self.tree))) , {node.pose:node.cost for node in self.tree}, False)
        for candidate in self.tree:
            pass
            #print(candidate.pose.robot.floating_point_pose, candidate.pose.obstacle.floating_point_pose, self.collision_free(candidate))
        return (False, [], self.tree, set(list(map(lambda x : x.pose, self.tree))), {node.pose:node.cost for node in self.tree}, False)
