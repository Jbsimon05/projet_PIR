...
class ClearLocalCostmap(py_trees.behaviour.Behaviour):
    def __init__(self, node: NamoBehaviorNode):
        super().__init__(name="ClearLocalCostmap")
        self.node = node
    ...
    def callback(self, future: Future):
        try:
            ...
            self.node.get_logger().error(f"ClearLocalCostmap failed with error: {e}")
            self._status = Status.FAILURE

    def initialise(self):
        self._status = Status.RUNNING
        future = self.node.clear_local_costmap()
        future.add_done_callback(self.callback)

    def update(self):
        if self._status == Status.RUNNING:
            self.node.get_logger().info("Clearing costap")
