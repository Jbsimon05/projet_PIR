...
class BasicNavigator(Node):
    def __init__(self, node_name="basic_navigator", namespace="/"):
        super().__init__(
            cli_args=["use_sim_time", "True"], node_name=node_name, namespace=namespace
        )
        self.initial_pose = PoseStamped()
        self.initial_pose.header.frame_id = "map"
        self.initial_pose.header.stamp = self.get_clock().now().to_msg()
        self.get_logger().info(f"Now: {self.get_clock().now().to_msg()}")
        ...
        self.initial_pose_pub = self.create_publisher(
            PoseWithCovarianceStamped, "initialpose", amcl_pose_qos
        )
    ...
    def followWaypoints(self, poses):
        ...
        while not self.follow_waypoints_client.wait_for_server(timeout_sec=30.0):
